...
nums: string; orderTitle: string; _id: string; netTypeLabel: string; address: string; orderStsLabel: string; ctime: string; 
  ...
  if(items.orderSts ==1){stsLabel = "label-light-success"; orderTips = "验证完成";
  }else if(items.orderSts ==2){stsLabel = "label-light-info";  
  }else if(items.orderSts ==3){stsLabel = "label-light-warning";
  }else if(items.orderSts ==4){stsLabel = "label-light-danger";
  }else if(items.orderSts ==5){stsLabel = "label-light-danger";}
  ...
  <div class="list-box margintop10 bgwhite bordertop" v-for="(item,index) in recordList" :key="index" @click="()=>detailDialog(item)">
        <div class="left-area record-lineheight">
              ...
              <div  class="limit-wrap limit-words font666 font13 width260 lineellipsis">
                ...
                <div>状态：<span :class='["step-btn-padding","record-label","label-inline",
                item.orderSts ==1 ? "label-light-success":"",
                item.orderSts ==2 ? "label-light-info":"",
                item.orderSts ==3 ? "label-light-warning":"",
                item.orderSts ==4 ? "label-light-danger":"",
                item.orderSts ==5 ? "label-light-danger":""]'>
                  {{item.orderStsLabel}}</span></div>
